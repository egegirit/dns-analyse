...
def read_single_pcap(pcap_file_name, current_pl_rate):
    ...
    global ip_false_answer
    global pl_dict
    ip_false_answer = {}
    pl_dict = {}

    if current_pl_rate not in pl_dict:
        pl_dict[current_pl_rate] = {}
    ip_rcode_latency_dict = {}
    if current_pl_rate not in ip_false_answer:
        ip_false_answer[current_pl_rate] = {}
    ...
    for packet in PcapReader(pcap_file_name):
                ...
                # Packet is a DNS response
                    ...
                    if (query_name, 0) in first_latency_queries and rcode == 0 and answer_count > 0 \
                        ...
                        if "client" in pcap_file_name:
                            wrong_answer = True
                            wrong_answer_list = []
                            for resource in packet[DNS].an:
                                if resource.type == 1:
                                    # This is an A record
                                    answer = resource.rdata
                                    # print(f"    Answer of {src_ip}: {answer}")
                                    if answer == "139.19.117.11":
                                        wrong_answer = False
                                        break
                                    else:
                            ...
                            if wrong_answer:
                                # print(f"========================")
                                # print(f"      Wrong answer from {src_ip}: {answer}")
                                # packet.show()
                                time.sleep(1)
                                if src_ip not in ip_false_answer[current_pl_rate]:
                                    ip_false_answer[current_pl_rate][src_ip] = []
                                for ans in wrong_answer_list:
                                    ip_false_answer[current_pl_rate][src_ip].append(ans)
                            # Only calculate latency from correct answers
                            else:
                                rcode_latency_tuple = (rcode, latency)
                                if src_ip not in ip_rcode_latency_dict:
                                    ip_rcode_latency_dict[src_ip] = [rcode_latency_tuple]
                                else:
                                    ip_rcode_latency_dict[src_ip].append(rcode_latency_tuple)
        ...
        #     print(f"      Breaking: ({datetime.now()}) {index}")
        #     break
    ...
    # Pickle
    pl_dict[current_pl_rate] = ip_rcode_latency_dict
...
def extract_data_from(file_name, pcap_file_prefix):
    ...
    for current_pl_rate in packetloss_rates:
        ...
        false_answers_name = f"ip_false_answer_{current_pl_rate}"

        save_dictionary(false_answers_name, ip_false_answer)

        f = open(f"{false_answers_name}.txt", "w")
        f.write(str(ip_false_answer))
        f.close()

        pl_dict_name = f"pl_dict_{current_pl_rate}"

        save_dictionary(pl_dict_name, pl_dict)

        f = open(f"{pl_dict_name}.txt", "w")
        f.write(str(pl_dict))
        f.close()

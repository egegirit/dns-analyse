...
sleep_time = 1
# Time to sleep between packetloss configurations. (300 seconds = 5 minutes)
sleep_time_between_packetloss_config = 300
...
directory_name_of_logs = "capture_logs_client_tcp"
...
def build_and_send_query_mp(args_list):
    ...
    for c in range(counter_min, counter_max):
        ...
        print(f"      Sending Query {query_prefix}\n        (Packetloss rate: {str(packetloss_rate)},\tIP: {ip_addr},"
              f"\tCounter: {c})")
        try:
            answers = resolver.resolve(query, "A", tcp=True)
        except Exception:
...
for current_packetloss_rate in packetloss_rates:
    ...
    # Sleep for 5 minutes between packetloss configurations

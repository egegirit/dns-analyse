...
def is_query_name_valid(query_name):
    query_pattern = "^(\d+)-(\d+)\.ripeatlas-pl([0-9]{1,2})-([0-9]{1,2})\.packetloss\.syssec-research\.mmci\.uni-saarland\.de\.$"
    search_result = re.search(query_pattern, query_name, re.IGNORECASE)
    if search_result is None:
        # print(f"Invalid query name: {query_name}")
        return False
    else:
        probe_id = search_result.group(1)
        time_stamp = search_result.group(2)
        pl_rate = search_result.group(3)
        counter = search_result.group(4)
        return [probe_id, time_stamp, pl_rate, counter]
    ...
    for packet in PcapReader(pcap_file_name):
        ...
        if packet.haslayer(DNS):
            ...
            try:
                ...
                extracted_result = is_query_name_valid(query_name)
                if not extracted_result:
                    # print(f" Query name does not match: {query_name}")
                    continue
                else:
                    probe_id = extracted_result[0]
                    time_stamp = extracted_result[1]
                    pl_rate_of_packet = extracted_result[2]
                    counter = extracted_result[3]
...
extract_datas_from_pcap("OverallBehaviour")

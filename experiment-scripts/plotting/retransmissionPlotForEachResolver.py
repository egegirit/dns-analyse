...
def create_bar_plot_failure(directory_name, file_name, operator_specific_packet_list, bottom_limit, upper_limit):
    ...
    f.write(f"Creating failure bar plot for {operator_name}\n")
...
def create_bar_plot_retransmission(directory_name, file_name, bottom_limit, upper_limit, operator_specific_packet_list, use_limits=False):
    ...
    f.write(f" Creating retransmission bar plot: {file_name}\n")
    ...
    f.write(f"Creating retransmission  bar plot for {operator_name}\n")
    ...
    keys = list(failure_rate_data_dict.keys())
    values = list(failure_rate_data_dict.values())
    print(f"{file_name} Retransmission rates: {keys}")
    f.write(f"{file_name} Retransmission rates: {keys}\n")
    print(f"{file_name} Retransmission counts: {values}")
    f.write(f"{file_name} Retransmission counts: {values}\n")
    ...
    plt.title(f"Retransmission Count for {operator_name}")
...
def calculate_retransmission_of_query(current_packet, packetloss_index, file_name):
    ...
    print(f" Count of total filtered packets: {len(packets)}")
    f.write(f" Count of total filtered packets: {len(packets)}\n")
    # DEBUG
    ...
    if queries_count > 1:
        if debug:
            ...
            f.write(f"  {file_name}: Multiple ({queries_count}) queries for: {query_name_of_packet}\n  Response count of it: {responses_count}\n  Packetloss index of it: {packetloss_index}\n")
            for query in queries:
                print(f"    Found query names: {extract_query_name_from_packet(query)}")
                f.write(f"    Found query names: {extract_query_name_from_packet(query)}\n")
                print(f"    Frame time of query: {get_frame_time_relative_of_packet(query)}")
                f.write(f"    Frame time of query: {get_frame_time_relative_of_packet(query)}\n")

            for resp in responses:
                print(f"    Found response names: {extract_query_name_from_packet(resp)}")
                f.write(f"    Found response names: {extract_query_name_from_packet(resp)}\n")
                print(f"    Frame time of response: {get_frame_time_relative_of_packet(resp)}")
                f.write(f"    Frame time of response: {get_frame_time_relative_of_packet(resp)}\n")
        ...
        print(f"      Duplicate count: {duplicate_query_count}")
        f.write(f"      Duplicate count: {duplicate_query_count}\n")
...
def loop_all_packets_for_retransmission(packet_list, file_name):
    ...
    # f.write("Looping packets to add retransmission counts\n")
    # global all_packets
    # for packet in all_packets:
    #     pass
    for packet in packet_list:
...
def run_with_filters():
    ...
    all_possible_rcodes = [rcodes1]

    # Define limits of the plots
    bottom_limit = 0
    upper_limit = 50

    directory_names = ["retransmit-test"]
    ...
    for rcode_filter in all_possible_rcodes:
        ...
        for file_name in file_names:
            ...
            for operator in list_of_operators:
                ...
                # create_box_plot(directory_names[0], file_name, operator, rcode_filter, bottom_limit, upper_limit, log_scale_y_axis)
                # create_bar_plot_failure(directory_names[0], file_name, operator, bottom_limit, 100)
                # create_violin_plot(directory_names[0], file_name, operator, bottom_limit, upper_limit, log_scale_y_axis)
                loop_all_packets_for_retransmission(operator, file_name)
                create_bar_plot_retransmission(directory_names[0], file_name, bottom_limit, upper_limit, operator, use_limits=False)
...
f = open("retransmission-output.txt", "a")
